    ...
    try:
       ...
       #split是用空格分開兩個str
        if (isInputCostInt(Input_data = dailyCost.split(' ')) is True) and (len(dailyCost.split(' ')) == 3):
            ...
            #cursor.execute("CREATE TABLE userdata (id serial PRIMARY KEY, time ,name VARCHAR(50), userid VARCHAR(50));")    
            "CREATE TABLE userdata (id serial PRIMARY KEY, time VARCHAR(50) ,name VARCHAR(50), userid VARCHAR(50));  "
